...
// import helper.NoteLoader.NoteEK;
...
class Controls extends FlxActionSet
{
	...
	// New
	var _noteP:Vector<FlxActionDigital> = new Vector(NoteList.buttonLength);
	var _noteQ:Vector<FlxActionDigital> = new Vector(NoteList.buttonLength);
	var _noteR:Vector<FlxActionDigital> = new Vector(NoteList.buttonLength);
	inline static final _notePrefix:String = "note_";
	inline static final _noteSuffixP = "-press";
	inline static final _noteSuffixQ = "-release";
	inline public function noteCheckerP (key:Int):Void->Bool return _noteP[key].check;
	inline public function noteCheckerQ (key:Int):Void->Bool return _noteQ[key].check;
	inline public function noteCheckerR (key:Int):Void->Bool return _noteR[key].check;
	//
	...
	public function new(name, scheme = None)
	{
		...
		// New
		for (num => str in NoteList.buttons) {
			_noteP[num] = new FlxActionDigital(_notePrefix + str + _noteSuffixP);
			add(_noteP[num]);
			_noteQ[num] = new FlxActionDigital(_notePrefix + str);
			add(_noteQ[num]);
			_noteR[num] = new FlxActionDigital(_notePrefix + str + _noteSuffixQ);
			add(_noteR[num]);
		}
		//
	...
	function forEachBound(control:Control, func:FlxActionDigital->FlxInputState->Void)
	{
		switch (control)
		{
			...
			//}}}
			case NOTE_(ek):
				func(_noteP[ek], JUST_PRESSED);
				func(_noteQ[ek], PRESSED);
				func(_noteR[ek], JUST_RELEASED);
			case ACCEPT:
	...
	public function setKeyboardScheme(scheme:KeyboardScheme, reset = true)
	{
		...
		switch (scheme)
		{
			case Solo:
				...
				//}}}
				for (num => str in NoteList.buttons) {
					inline bindKeys(Control.NOTE_(num), keysMap.get(_notePrefix + str));
				}
			...
			// These aren't really used (or usable) for now
			// case Duo(true):
	...
	public function addDefaultGamepad(id):Void
	{
		...
		addGamepadLiteral(id, [
			...
			//{{{
			Control.NOTE_(0) => [DPAD_LEFT, LEFT_STICK_DIGITAL_LEFT, RIGHT_STICK_DIGITAL_LEFT, X],
			Control.NOTE_(1) => [DPAD_DOWN, LEFT_STICK_DIGITAL_DOWN, RIGHT_STICK_DIGITAL_DOWN, A],
			Control.NOTE_(2) => [DPAD_UP, LEFT_STICK_DIGITAL_UP, RIGHT_STICK_DIGITAL_UP, Y],
			Control.NOTE_(3) => [DPAD_RIGHT, LEFT_STICK_DIGITAL_RIGHT, RIGHT_STICK_DIGITAL_RIGHT, B],
			//}}}
			Control.PAUSE => [START],
			Control.RESET => [8]
		]);
		...
		addGamepadLiteral(id, [
			...
			//{{{
			Control.NOTE_(0) => [DPAD_LEFT, LEFT_STICK_DIGITAL_LEFT, RIGHT_STICK_DIGITAL_LEFT, Y],
			Control.NOTE_(1) => [DPAD_DOWN, LEFT_STICK_DIGITAL_DOWN, RIGHT_STICK_DIGITAL_DOWN, B],
			Control.NOTE_(2) => [DPAD_UP, LEFT_STICK_DIGITAL_UP, RIGHT_STICK_DIGITAL_UP, X],
			Control.NOTE_(3) => [DPAD_RIGHT, LEFT_STICK_DIGITAL_RIGHT, RIGHT_STICK_DIGITAL_RIGHT, A],
			//}}}
			Control.PAUSE => [START],
			Control.RESET => [8],
		]);
	...
	#if mobile
	/**
	 * Add FlxTouchInput to Action trigger.
	 */
	public function bindTouchInput(control:Control, input:FlxTouchInput)
	{
		inline forEachBound(control, (action, state) -> action.add(new FlxActionInputDigitalIFlxInput(input, state)));
	}

	/**
	 * Remove all FlxTouchInput from Action.
	 * (Probably not used)
	 */
	public function flushTouchInput()
	{
		for (action in digitalActions) {
			var i = action.inputs.length;
			while (i-- > 0)
			{
				var input = action.inputs[i];
				if (input.device == FlxInputDevice.IFLXINPUT_OBJECT)
					action.remove(input);
			}
		}
	}
