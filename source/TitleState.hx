...
class TitleState extends MusicBeatState
{
	...
	override public function create():Void
	{
		...
		//trace(path, Paths.exists(path));

		/*#if (polymod && !html5)
		if (Paths.exists('mods/')) {
	...
	function startIntro()
	{
		...
		//trace(path, Paths.exists(path));
		titleText.frames = FlxAtlasFrames.fromSparrow(BitmapData.fromFile(path), Paths.getText(StringTools.replace(path,".png",".xml")));
		#else
		...
		if(!closedState) {
			...
			switch (sickBeats)
			{
				case 1:
					...
					createCoolText(['Game made by', 'ninjamuffin99 etc'], -50);
				case 2:
					addMoreText('Engine made by', 15);
					addMoreText('ShadowMario etc', 15);
				case 4:
					addMoreText('', 15);
					addMoreText('Idk', 15);
					addMoreText('Go read credits', 15);
				case 5:
					deleteCoolText();
