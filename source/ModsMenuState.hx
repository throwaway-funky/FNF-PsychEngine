...
class ModsMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		removeButton = new FlxButton(startX, 620, "Delete Selected Mod", function()
		{
			...
			if(Paths.exists(path) && FileSystem.isDirectory(path))
			{
		...
		while (i < modsList.length)
		{
			...
			if(!Paths.exists(Paths.mods(values[0])))
			{
	...
	function onLoadComplete(_):Void
	{
		...
		if(fullPath != null)
		{
			var rawZip:String = Paths.getText(fullPath);
			if(rawZip != null)
			{
