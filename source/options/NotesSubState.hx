...
class NotesSubState extends MusicBeatSubstate
{
	...
	private static var curScroll:Int = 0;
	var posX = 230;
	static final rowH = 165;
	public function new() {
		super();
		...
		var refNote = new FlxSprite();
		refNote.frames = Paths.getSparrowAtlas('NOTE_assets');
		NoteLoader.loadNoteAnimsByKeyScheme([for (i in 0...NoteEK.TOTAL_DEFINED) i], refNote, [' '], ['']);
		for (i in 0...NoteEK.TOTAL_DEFINED) {
			var posY:Float = (rowH * i) + 35;
			for (j in 0...3) {
				var optionText:Alphabet = new Alphabet(0, posY + 60, Std.string(ClientPrefs.arrowHSV[i][j]), true);
				optionText.x = posX + (225 * j) + 250;
				grpNumbers.add(optionText);
			}

			var note:FlxSprite = new FlxSprite(posX, posY);
			// note.frames = Paths.getSparrowAtlas('NOTE_assets');
			// var animations:Array<String> = ['purple0', 'blue0', 'green0', 'red0', 'red0', 'red0']; // Temporary
			// note.animation.addByPrefix('idle', animations[i]);
			// note.animation.play('idle');
			note.frames = refNote.frames;
			note.animation.add('idle', refNote.animation.getByName(NoteList.keys[i].id).frames);
			note.animation.play('idle');
	...
	function changeSelection(change:Int = 0) {
		...
		if (curSelected < curScroll) curScroll = curSelected;
		if (curScroll < curSelected - 3) curScroll = curSelected - 3;
		...
		for (i in 0...grpNumbers.length) {
			...
			item.y = (rowH * (Std.int(i/3) - curScroll)) + 95;
		}
		for (i in 0...grpNotes.length) {
			var item = grpNotes.members[i];
			item.alpha = 0.6;
			item.scale.set(0.75, 0.75);
			item.y = (rowH * (i - curScroll)) + 35;
			if (curSelected == i) {
				item.alpha = 1;
				item.scale.set(1, 1);
				hsbText.y = item.y - 70;
