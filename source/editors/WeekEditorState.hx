...
class WeekEditorState extends MusicBeatState
{
	...
	function reloadBG() {
		...
		if(assetName != null && assetName.length > 0) {
			if( #if MODS_ALLOWED Paths.exists(Paths.modsImages('menubackgrounds/menu_' + assetName)) || #end
			Paths.exists(Paths.getPath('images/menubackgrounds/menu_' + assetName + '.png', IMAGE))) {
	...
	function reloadWeekThing() {
		...
		var isMissing:Bool = true;
		if(assetName != null && assetName.length > 0) {
			if( #if MODS_ALLOWED Paths.exists(Paths.modsImages('storymenu/' + assetName)) || #end
			Paths.exists(Paths.getPath('images/storymenu/' + assetName + '.png', IMAGE))) {
	...
	private static function onLoadComplete(_):Void
	{
		...
		if(fullPath != null) {
			var rawJson:String = Paths.getText(fullPath);
			if(rawJson != null) {
